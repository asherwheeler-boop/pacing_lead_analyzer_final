...
def reconstruct_3d_wire(front_pts, side_pts, n=N_INTERP):
    """Reconstruct 3-D wire from biplane views.

    Convention (biplane X-ray):
      Front view -> X (col 0) = lateral,   Y (col 1) = cranio-caudal (shared axis)
      Side  view -> X (col 0) = A-P depth, Y (col 1) = cranio-caudal (shared axis)

    The three 3-D coordinates are:
      X_3d = front lateral   (front col 0)
      Y_3d = cranio-caudal   (front col 1)
      Z_3d = A-P depth       (side  col 0)
    """
    sf = _arc_length_2d(front_pts); sf /= sf[-1]
    ss = _arc_length_2d(side_pts);  ss /= ss[-1]
    t = np.linspace(0, 1, n)
    return np.column_stack([CubicSpline(sf, front_pts[:, 0])(t),   # X: lateral
                            CubicSpline(sf, front_pts[:, 1])(t),   # Y: cranio-caudal
                            CubicSpline(ss, side_pts[:, 0])(t)])   # Z: A-P depth
...
def generate_3d_plotly(results, unit_label="\u00b5m"):
    ...
    for i, (cname, res) in enumerate(sorted(results.items())):
        ...
        # Exhale: Scatter3d does not support dash; use reduced opacity to distinguish
        fig.add_trace(go.Scatter3d(x=w_ex[:, 0], y=w_ex[:, 1], z=w_ex[:, 2],
            mode="lines", name=cname + " Exhale",
            line=dict(width=4, color=c_ex), opacity=0.55))
...
def generate_html_report(output):
    ...
    notes = _html.escape(output.get("patient_notes", ""))
